...
const calculateMeshVolume = (mesh: THREE.Mesh) => {
	let sum = 0;
	const {
		matrixWorld,
		geometry: {
...
export const calculateCadVolume = (cad: THREE.Group) => {
	let volume = 0;
	cad.traverse((child) => {
		// non-mesh geometry-containing objects are intentionally skipped
